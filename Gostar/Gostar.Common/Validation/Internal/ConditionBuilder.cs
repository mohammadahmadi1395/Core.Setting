...
namespace Gostar.Common.Validation.Internal {
    ...
    internal class ConditionBuilder<T> {
        ...
        bool Condition(ValidationContext context, Func<T, bool> predicate)
        {
            ...
            if (context.InstanceToValidate != null)
            {
                ...
                object value;
                if (context.RootContextData.TryGetValue(cacheId, out value))
                {
                    if (value is bool)
                    {
        ...
        public IConditionBuilder When(Func<T, bool> predicate, Action action) {
            ...
            foreach (var rule in propertyRules) {
				...
				if (rule is PropertyRule ) {
                    var p = (PropertyRule)rule;

                    p.ApplySharedCondition(x=>Condition(x, predicate));
			...
			return new ConditionOtherwiseBuilder(_rules,x=> Condition(x, predicate));
		}
	...
	internal class AsyncConditionBuilder<T> {
        ...
        async Task<bool> Condition(ValidationContext context, CancellationToken ct)
        {
            ...
            var executionResult = await pre((T)context.InstanceToValidate,ct);
            if (context.InstanceToValidate != null)
            {
                context.RootContextData[cacheId] = executionResult;
            }
        ...
        public Func<T, CancellationToken, Task<bool>> pre;
        /// <summary>
