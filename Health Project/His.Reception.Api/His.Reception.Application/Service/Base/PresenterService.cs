...
namespace His.Reception.Application.Service.Base
{
    public class PresenterService : IPresenterService
    {
        ...
        private readonly DbSet<Presenter> _presenterRepository;
        public PresenterService(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
            _presenterRepository = _unitOfWork.Set<Presenter>();
        }

        public async Task<BaseResponseDto> GetAll()
        {
            var lstEducation = await _presenterRepository.Select(g => Mapper.BaseMapper.Map(g)).ToListAsync();
